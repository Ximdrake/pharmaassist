...
<div class="mdl-card mdl-shadow--2dp mdl-cell mdl-cell--12-col mdl-cell--8-col-tablet mdl-cell--12-col-desktop margin-top-0">
    <div class="mdl-card__title mdl-color--primary mdl-color-text--white">
        <h2 class="mdl-card__title-text logo-style">
            @if ($totalUsers === 1)
                {{ $totalUsers }} Patient total
            @elseif ($totalUsers > 1)
                {{ $users->count() }} Total Patients
            @else
                No Users :(
            @endif
        </h2>
 ...
 <div class="mdl-card__supporting-text mdl-color-text--grey-600 padding-0 context">
        <div class="table-responsive material-table">
            <table id="patient_table" class="mdl-data-table mdl-js-data-table data-table" cellspacing="0" width="100%">
              <thead>
                <tr>
                    <th class="mdl-data-table__cell--non-numeric">Contact Number</th>
                    <th class="mdl-data-table__cell--non-numeric">Gender</th>
                    <th class="mdl-data-table__cell--non-numeric mdl-layout--large-screen-only">Patient Name</th>
                    <th class="mdl-data-table__cell--non-numeric mdl-layout--large-screen-only">Doctor</th>
              ...
              <tbody>
                    @foreach ($users as $user)
                        <tr>
                            <td class="mdl-data-table__cell--non-numeric"><a href="{{ URL::to('users/' . $user->id) }}">{{$user->contact_number}} </a></td>
                            <td class="mdl-data-table__cell--non-numeric"><a href="{{ URL::to('users/' . $user->id) }}">{{$user->gender}} </a></td>
                            <td class="mdl-data-table__cell--non-numeric mdl-layout--large-screen-only"><a href="{{ URL::to('users/' . $user->id) }}">{{$user->firstname}} {{$user->lastname}} </a></td>
                            <td class="mdl-data-table__cell--non-numeric mdl-layout--large-screen-only"><a href="{{ URL::to('users/' . $user->id) }}">Dr. {{$user->docfirstname}} {{$user->doclastname}} </a></td>
                            <td class="mdl-data-table__cell--non-numeric">
                                
                                    @if ($user->status == 'On Maintenance')
                                        @php
                                            $levelIcon        = 'person';
                                            $levelName        = 'On Maintenance';
                            ...
                            <td class="mdl-data-table__cell--non-numeric">
                                ...
                                <a href="{{ URL::to('users/' . $user->id) }}" class="mdl-button mdl-button--icon mdl-js-button mdl-js-ripple-effect" title="View User Profile">
